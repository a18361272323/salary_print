(function (root, factory) {
  ...
  function renderPrintPages(input) {
    ...
    return pages.map(function (page, index) {
      ...
      var header = isFirst ? '<div class="print-title"><h2>' + escapeHtml(config.title) + "</h2><p>" + escapeHtml(config.groupName) + "　" + escapeHtml(config.cycleName) + "</p></div>" : '<div class="continuation-title">工资表（续表）<span>' + escapeHtml(config.groupName) + "　" + escapeHtml(config.cycleName) + "</span></div>";
