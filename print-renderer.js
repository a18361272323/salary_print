(function (root, factory) {
  ...
  function renderPrintPages(input) {
    ...
    return pages.map(function (page, index) {
      ...
      return '<section class="print-page print-page--' + page.kind + '">' + header + '<table>' + renderColumnWidths(columns) + '<thead>' + renderGroupHeader(columns) + '<tr class="column-header">' + columns.map(function (column) { return "<th>" + escapeHtml(column.label) + "</th>"; }).join("") + "</tr></thead><tbody>" + rows + "</tbody>" + summary + "</table>" + signature + '<div class="page-footer">第 ' + (index + 1) + " 页 / 共 " + pages.length + " 页　第 " + (page.rows.length ? page.rows[0]._sequence : 0) + "–" + (page.rows.length ? page.rows[page.rows.length - 1]._sequence : 0) + " 条 / 共 " + (config.totalRows || 0) + " 条</div></section>";
    }).join("");
  }
