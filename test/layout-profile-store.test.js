...
test("save updates the matching global profile with widths excluded and version incremented", async () => {
  const calls = [];
...
test("save rejects blank or missing owner values before writing", async () => {
  for (const ownerUserNo of ["", "   ", undefined, null]) {
    ...
    await assert.rejects(store.save({ scope: "global", layout: {} }), /owner/i);
    assert.equal(calls, 0);
  }
});
