...
开始
  ...
  → 提取旧记录ID（JS脚本）
  → 有旧记录吗（分支容器）
      ├─ 替换已有配置（条件：`${hasOldRecords}`）
      │    └─ 数据库事务
      │        ├─ 删除已有列配置（模型方法/批量删除）
      │        └─ 批量写入新配置（SQL脚本 + foreach）
      └─ 其他（else） → 首次批量写入（SQL脚本 + foreach）
  → 两条分支汇合
  → 回读保存结果（模型方法/列表查询）
  → 组装最终输出（JS脚本）
  → 结束
...
| 有旧记录吗 | branch | 仅作为分支容器；标题为“有旧记录吗” | 无 | 无 | 承载条件子节点与 else 槽位 | 当前截图已确认 |
| 替换已有配置 | condition | 分支内条件子节点；表达式为 `${hasOldRecords}`；忽略错误关 | `state.hasOldRecords` | 无 | true 时进入事务；false 时进入 else | 当前截图已确认 |
...
1. 在“提取旧记录ID”后增加“分支”。
2. 节点标题填：`有旧记录吗`。
3. 这个“分支”节点只负责容器和标题；右侧配置面板只有标题、忽略错误开关是正常现象，**不要**在这里填写 `hasOldRecords`。
4. 点击分支容器右侧或内部的“+”，新增一个“条件”子节点。
5. 选中该条件子节点，节点标题填：`替换已有配置`。
6. 条件表达式填：`${hasOldRecords}`。
7. 两个节点的“是否忽略该节点执行错误”都保持关闭。
8. 该条件子节点的 true 路径用于第 4.6 节数据库事务；它的“其他（else）”路径用于第 4.7 节首次 SQL 写入。

你截图中“有旧记录吗（分支）”右侧的“替换已有配置（条件）”及表达式 `${hasOldRecords}` 正是正确结构，不能删除。分支只会把布尔值 `true` 送入该条件的 true 路径；文本 `"true"` 不算。`提取旧记录ID`已写入布尔值 `state.hasOldRecords`，因此不需要字符串比较。

分支只会把布尔值 `true` 送入真分支；文本 `"true"` 不算。`提取旧记录ID`已写入布尔值 `state.hasOldRecords`，因此不要在条件节点里重复做字符串比较。一次只执行一个分支。

### 4.6 真分支：替换已有配置（数据库事务）

#### 4.6.1 新建事务外壳

1. 点击“替换已有配置”这个**条件子节点下方的蓝色“+”**，选择“数据库事务”。不要点击画布主干下方的加号。
2. 节点标题填：`替换已有配置`。
