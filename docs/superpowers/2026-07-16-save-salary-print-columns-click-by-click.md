...
1. 在“提取旧记录ID”后增加“分支”。
2. 节点标题填：`有旧记录吗`。
3. 条件表达式通过“内置变量使用说明”的变量选择器插入 `hasOldRecords`；以编辑器自动插入的形式为准，不要手工把它改成另一种包裹语法。
4. 该节点右侧会出现“真分支”和“其他（else）”两条**分支槽位**。它们不是已经配置好的业务节点。
5. 在真分支对应的“+”中选择第 4.6 节的“数据库事务”；在 else 分支对应的“+”中选择第 4.7 节的“SQL脚本”。
6. 不要在真分支再选择“条件”。若画布上出现一个带灰色条件图标、名称为“替换已有配置”的节点，说明误新增了一层条件，应删除该节点后按第 5 步重新新增“数据库事务”。
7. 在调试日志中必须能看到空旧记录时走 else 分支。
...
1. 点击“有旧记录吗”真分支槽位内的“+”，选择“数据库事务”。不要点击画布主干下方的加号，也不要选择“条件”。
2. 节点标题填：`替换已有配置`。
...
2. 假分支：点击“有旧记录吗”中“其他（else）”槽位对应的“+”，选择“SQL脚本”，标题填 `首次批量写入`；不要在画布主干另起一个 SQL 节点。
