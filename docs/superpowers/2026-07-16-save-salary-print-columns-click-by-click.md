...
不要给删除、事务或 SQL 写入打开“忽略错误”。有旧记录时，删除和批量 INSERT 必须放在同一个数据库事务节点内；官方规则是事务内所有步骤全部成功或全部失败。
...
- 列表查询实际返回是否为 `{ list, total, current, pageSize }`。
- 分支布尔条件的编辑器选择方式和日志回显。
...
4. 真分支：连接“替换已有配置”数据库事务节点。
5. 假分支：连接“首次批量写入”SQL脚本节点。
...
2. 节点标题填：`替换已有配置`。
3. 数据源选择“工资表打印列配置”所在的同一个内置数据源；必须与第 4.7 节 SQL 的数据源相同。
...
批量删除必须只出现在真分支的事务内部。不要把空数组或缺失的 `ids` 交给批量删除。事务第 2 步就是第 4.7 节“批量写入新配置”。官方规则是任一步失败时整组事务失败；仍要通过失败注入和数据回读确认目标配置无误。

### 4.7 批量写入新配置（SQL脚本 + foreach）

#### 4.7.1 先从 SQL 编辑器读取真实标识

1. 真分支：在“替换已有配置”事务内部、删除步骤后增加“SQL脚本”，标题填 `批量写入新配置`。
2. 假分支：直接增加一个“SQL脚本”，标题填 `首次批量写入`。
3. 两个 SQL 节点选择与目标模型相同的内置数据源。
4. 打开 SQL 编辑器的模型/表/字段选择器，找到“工资表打印列配置”。
5. 把真实物理表名记为 `{salaryPrintColumnTable}`；逐一核对下方 SQL 中的字段标识。若选择器给出的标识不同，全部替换为选择器值。
6. 不要把模型中文名直接当表名，不要从其他环境复制表名。

#### 4.7.2 两个 SQL 节点都粘贴同一份 SQL
...
| 节点标题 | `批量写入新配置` | `首次批量写入` |
| 数据源 | 与模型和事务相同 | 与模型和事务相同 |
| 存入变量 | `insertResult` | `insertResult` |
| 自动分页 | 关闭；INSERT 不分页 | 关闭；INSERT 不分页 |
| 忽略错误 | 关闭 | 关闭 |
| 预期日志 | 一次 SQL 执行，参数集合为全部 `validatedColumns` | 一次 SQL 执行，参数集合为全部 `validatedColumns` |
| 下游节点 | 分支汇合后的“回读保存结果” | 分支汇合后的“回读保存结果” |

如果 SQL 编辑器不接受 `${item.field}` 或集合名，先在调试参数放两条 `columns`，打开该 SQL 节点日志，按日志提示修正集合路径；不要改成循环逐条写入来掩盖路径错误。SQL 的 `foreach` 支持数组/List/Set/Map，本流程传入的是数组。

#### 4.7.3 宽度字段的处理

当前输入不维护 `width_mm`，所以 SQL 中暂不写该字段，让模型/数据库默认值生效。如果目标物理表要求非空且没有默认值，先从字段配置读出默认值，再把 `width_mm` 加到字段列表和每个 VALUES 元组；不要凭经验填数字。

#### 4.7.4 兼容回退：目标 SQL 编辑器确实无法绑定数组时

只有在调试日志证明当前环境的 SQL 节点无法读取 `validatedColumns` 时，才回退到“循环 + 模型新增”：循环源使用裸变量 `validatedColumns`，项变量 `_item`，索引 `_index`；循环内选择 `内置数据库` → `工资表打印列配置` → `新增`，字段映射沿用下表。该回退不能与批量删除组成官方已确认的单 SQL 一致性，必须保留事务并做失败注入。

循环内模型方法关闭“过滤空值”和“忽略错误”，请求参数配置：

| 模型字段 | 参数值 | 值来源 |
| --- | --- | --- |
| `config_scope` | `personal` | 常量 |
| `owner_user_no` | `${xcUser.userNo}` | 服务端当前用户 |
| `profile_type` | `column` | 常量 |
| `salary_group_id` | `${salaryGroupId}` | 校验后变量 |
| `salary_cycle` | `${salaryCycle}` | 校验后变量 |
| `column_key` | `${_item.columnKey}` | 当前循环项 |
| `print_flag` | `${_item.printFlag}` | 当前循环项 |
| `display_order` | `${_item.displayOrder}` | 当前循环项 |
| `top_group` | `${_item.topGroup}` | 当前循环项 |
| `second_group` | `${_item.secondGroup}` | 当前循环项 |
| `total_flag` | `${_item.totalFlag}` | 当前循环项 |
| `enabled` | `1` | 常量 |
| `column_label_override` | 空字符串 | 当前公共输入不维护标签覆盖 |
| `width_mm` | 留空或模型默认 | 列宽由布局配置维护；写入行为需调试确认 |
| `vertical_text` | `0` | 常量 |
| `align_mode` | `center` | 常量 |
| `data_type` | `text` | 常量 |
| `mask_flag` | `0` | 常量 |
| `sort_priority` | `0` | 常量 |
| `sort_direction` | `ascending` | 常量 |
| `remark` | 空字符串 | 常量 |

如果新增方法的请求参数表中没有某个可选字段，不要自行新增字段；只配置选择器实际回显的参数。这个循环方案是回退路线，不是默认主链路。
...
| 循环 | 裸数组路径、项变量、索引变量 | 每次迭代上下文 | 仅兼容回退 | 默认项 `_item`、索引 `_index`；只在 SQL 数组绑定确实失败时使用 |
...
官方能力已经明确：数据库事务节点内的模型/SQL步骤顺序执行，所有步骤全部成功或全部失败。因此有旧数据的真分支必须配置为：
...
不要把分支、并序、异步或循环塞进事务；本方案只使用官方明确的顺序模型/SQL步骤。假分支没有旧数据，只执行一个 SQL 批量 INSERT；单个 SQL 节点自身具有事务一致性。

仍需调试确认的是目标数据源、物理表/字段标识、事务日志和 SQL 返回形态，而不是“是否全成全败”。测试时在 INSERT 中临时制造一个无效字段或违反约束的值，确认真分支报错后旧记录仍完整存在，然后立刻恢复正确 SQL。该失败注入只在开发环境执行。
...
4. 查询不到旧记录：走分支假路径，不调用批量删除，新增成功。
5. 在事务内 INSERT 制造错误：接口失败，旧记录仍完整存在，修复 SQL 后再次执行成功。
6. `columns` 含 3 条：SQL 日志只出现一次 INSERT 执行，最终回读为 3 条。
6. 正常保存后：`savedCount` 等于最终回读列表长度，且 `column_key` 集合与请求一致。
...
- [ ] 两个 SQL 节点的数据源、物理表名和字段名均从当前目标 SQL 编辑器读取。
- [ ] 首次列表查询的 `existingRecords` 调试输出包含 `list`，或脚本已按实际输出形态调整。
- [ ] 批量删除调试输入明确包含非空 `ids`。
- [ ] 空旧记录样例不会执行批量删除。
- [ ] SQL 日志显示 `foreach` 接收全部 `validatedColumns`，一次批量 INSERT 完成。
...
| 9 | 批量写入 | 主链路使用 SQL `foreach`，循环模型新增仅为明确标注的回退路线 |
